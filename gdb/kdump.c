...
#define SYMBOL(var,name) do { var = lookup_symbol(name, NULL, VAR_DOMAIN, NULL).symbol; if (! var) { fprintf(stderr, "Cannot lookup_symbol(" name ")\n"); goto error; } } while(0)
...
static offset get_symbol_address(const char *sname)
{
	...
	ss = lookup_global_symbol(sname, NULL, ALL_DOMAIN).symbol;
	if (! ss) {
		ss = lookup_static_symbol(sname, ALL_DOMAIN).symbol;
		if (! ss) return NULL_offset ;
	}
	lang  = language_def (SYMBOL_LANGUAGE (ss));
	val = lang->la_read_var_value (ss, NULL, NULL);
	if (! val) {
		return NULL_offset;
	}
...
static offset get_symbol_value(const char *sname)
{
	...
	struct value *val;
	offset off;
	ss = lookup_global_symbol(sname, NULL, VAR_DOMAIN).symbol;
	if (! ss) {
		ss = lookup_static_symbol(sname, VAR_DOMAIN).symbol;
		if (! ss) return NULL_offset ;
	}
	lang  = language_def (SYMBOL_LANGUAGE (ss));
	val = lang->la_read_var_value (ss, NULL, NULL);
...
static offset get_percpu_offset(const char *varname, int ncpu)
{
	...
	do {
		...
		sym = lookup_global_symbol(buff, NULL, VAR_DOMAIN).symbol;
...
void kdumptest_file_command (char *filename, int from_tty)
{
	...
	CORE_ADDR addr;
	ss = lookup_global_symbol(sname, NULL, FUNCTIONS_DOMAIN).symbol;
	if (! ss) {
		return;
	}
	lang  = language_def (SYMBOL_LANGUAGE (ss));
	...
	ss = lookup_static_symbol("modules", VAR_DOMAIN).symbol;
	...
	} 
	{
		...
		struct value *val;
		struct obj_section *os;
		ss = lookup_global_symbol("runqueues", NULL, VAR_DOMAIN).symbol;
		if (! ss) {
			return;
		}
		printf("sect %d\n", SYMBOL_SECTION(ss));
